 ...
 let init_protos = do_once (fun () ->
   if Arge.get Arge.protos_test then (
     ...
     let config s vf =
       ...
       let ranking = Addr.default_ranking in
     ...
     *)
     ...
     let marsh,unmarsh = Iovecl.make_marsh send_pool true in
...
(* same as config_new, but returns the application-state *)
 let config_new_hack interface vf =
   let interface = 
     if Trace.log_check "APPL_TRACE" then
       (Appl_debug.f name interface)
     else
...
*)
     ...
     log_refcnt (fun () -> sprintf "stats=%s" (Iovec.get_stats ()))
   in
   ...
   let loop time =
     ...
     in
     Alarm.schedule (some_of name !sched_r) (Time.add time (Time.of_int next))
   in
 ...
 let main_loop () =
   ...
   while true do
     while !counter <= pollcount do
       ...
       (* Execute pending memory requests
        *)
       Socket.Iov.satisfy_pending ()
     done ;

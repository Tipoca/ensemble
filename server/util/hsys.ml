...
(**************************************************************)
let max_msg_size = Socket.max_msg_size
...
let udp_send sendto_info buf ofs len =
  Socket.udp_send sendto_info (Buf.string_of buf) (int_of_len ofs) (int_of_len len) 
...
(* PERF: I -hope- ocamlopt removes the allocation.
*)
let udp_mu_recv_packet pool sock mlhdr= 
(*  Socket.udp_mu_recv_packet sock (Buf.string_of mlhdr)*)
  let mllen, iov = Socket.udp_mu_recv_packet pool sock (Buf.string_of mlhdr) in
  (Buf.len_of_int mllen), iov
    
let tcp_recv s b o l 	= 
  len_of_int (Socket.tcp_recv s (Buf.string_of b) (int_of_len o) (int_of_len l))
    
let tcp_recv_iov s iov o l = 
  len_of_int (Socket.tcp_recv_iov s iov (int_of_len o) (int_of_len l))
    
let tcp_recv_packet sock buf ofs len iov = 
  len_of_int (Socket.tcp_recv_packet sock 
    (Buf.string_of buf) (int_of_len ofs) (int_of_len len) iov)
    
let tcp_send s b o l = 
  len_of_int (Socket.tcp_send s (Buf.string_of b) (int_of_len o) (int_of_len l))
    
let tcp_sendv s iovl = 
  len_of_int (Socket.tcp_sendv s (Iovecl.to_iovec_array iovl))
    
let tcp_sendsv s buf ofs len iovl = 
  len_of_int (Socket.tcp_sendsv s (Buf.string_of buf) 
    (Buf.int_of_len ofs) (Buf.int_of_len len) (Iovecl.to_iovec_array iovl))
    
let tcp_sends2v s buf1 buf2 ofs len iovl = 
  len_of_int (Socket.tcp_sends2v s
    (Buf.string_of buf1) 
